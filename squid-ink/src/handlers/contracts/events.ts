...
export async function handleContractInstantiated<P>({
  ctx,
  event,
  block,
...
export async function handleContractEmitted<P>({
  ctx,
  event,
  block,
}: EventHandlerParams<P>): Promise<void> {
  ...
  try {
    ...
    log.error(<Error>error, "Error handling contracts ContractEmitted event.");
  }
}

export async function handleContractCodeStored<P>({
  ctx,
  event,
  block,
}: EventHandlerParams<P>): Promise<void> {
  ...
  try {
    ...
    log.error(<Error>error, "Error while handling contract CodeStored event.");
  }
}

export async function handleContractCodeUpdated<P>({
  ctx,
  event,
  block,
}: EventHandlerParams<P>): Promise<void> {
